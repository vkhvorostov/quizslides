...
function renderPollSlide(state) {
    ...
    PANEL.innerHTML = `
        ...
        <p class="text-center text-muted small mb-3">Слайд ${state.current_slide} / ${state.total_slides}</p>
        <div class="mb-3">${optionsHtml}</div>
        <p class="text-center small ${voted ? 'text-success' : 'text-muted'} mb-0">
            ${voted ? 'Вы проголосовали. Можно выбрать другой вариант.' : 'Нажмите вариант, чтобы проголосовать'}
        </p>
    `;
